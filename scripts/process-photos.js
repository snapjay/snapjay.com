...
async function getImages(dir, relativePath = '') {
  ...
  for (const entry of entries) {
    ...
    } else if (entry.isFile() && /\.(jpg|jpeg|png|webp|avif|mp4|webm|mov|ogg)$/i.test(entry.name)) {
...
async function processPhotos() {
  try {
    ...
    for (let i = 0; i < images.length; i++) {
      ...
      const isVideoFile = /\.(mp4|webm|mov|ogg)$/i.test(inputPath);
      const outputFilename = isVideoFile 
        ? path.basename(relPath) 
        : path.basename(relPath, path.extname(relPath)) + '.webp';

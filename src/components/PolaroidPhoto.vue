...
const resolvedSrc = computed(() => transformCdnUrl(props.src));

const thumbSrc = computed(() => {
  let url = props.src;
  if (url && url.startsWith('/photos/') && url.endsWith('.webp')) {
    url = url.replace('.webp', '-thumb.webp');
  }
  return transformCdnUrl(url);
});
